...
const PROFILE = {
  name: process.env.AGENT_NAME || 'Sentinel-B',
  style: 'control-economy',
  aggression: Number(process.env.AGGRESSION || 0.46),
  economyBias: Number(process.env.ECONOMY_BIAS || 0.88),
  riskTolerance: Number(process.env.RISK_TOLERANCE || 0.38)
};
...
function predictOpponentAction(snapshot, session) {
  ...
  if (opponent.wallet.energy < config.attackCost) {
    if (opponent.wallet.data >= 2) {
      return { type: 'trade', give: 'data', receive: 'energy', amount: clamp(opponent.wallet.data, 1, 3) };
    }
    if (opponent.wallet.metal >= 2) {
      return { type: 'trade', give: 'metal', receive: 'energy', amount: clamp(opponent.wallet.metal, 1, 3) };
    }
    return { type: 'gather', resource: 'energy', amount: 2 };
  }

  const finalStretch = turn >= snapshot.config.maxTurns - 2;
  if (finalStretch || aggression > 0.58 || self.hp < 35) {
    return { type: 'attack', targetAgentId: self.agentId, amount: clamp(Math.max(1, Math.min(4, feasible)), 1, 10) };
  }
...
function buildCandidates(snapshot) {
  ...
  if (feasibleAttack > 0) {
    ...
    const ladder = [Math.ceil(feasibleAttack * 0.5), Math.ceil(feasibleAttack * 0.8), feasibleAttack]
  ...
  if (self.wallet.energy < config.attackCost * 2) {
    if (self.wallet.data >= 2) {
      pushUnique(candidates, {
        type: 'trade',
        give: 'data',
        receive: 'energy',
        amount: clamp(self.wallet.data, 1, 4)
      });
    }
  ...
  if (turnsRemaining > 2) {
    pushUnique(candidates, { type: 'gather', resource: 'data', amount: 2 });
    pushUnique(candidates, { type: 'gather', resource: 'metal', amount: 2 });
  }

  if (self.wallet.energy >= config.attackCost * 3 && (self.wallet.data < 5 || self.wallet.metal < 5)) {
    pushUnique(candidates, {
      type: 'trade',
      give: 'energy',
      receive: self.wallet.data <= self.wallet.metal ? 'data' : 'metal',
      amount: 1
    });
  }
...
function scoreState(state, snapshot, session) {
  ...
  const endgameMultiplier = turnsRemaining <= 2 ? 1.7 : 1;

  let value = 0;
  value += hpDiff * 7.4 * endgameMultiplier;
  value += scoreDiff * 2.15 * endgameMultiplier;
  value += energyDiff * 1.05;
  ...
  value -= incomingDamage * (1 - PROFILE.riskTolerance * 0.4);

  const aggressionSeen = predictedOpponentAggression(session);
  value -= aggressionSeen * 0.7;
...
function evaluateAction(snapshot, action, session) {
  ...
  if (action.type === 'attack') {
    score += action.amount * PROFILE.aggression * 1.1;
  }
  ...
  if (action.type === 'gather' && action.resource !== 'energy') {
    score += 0.35;
  }
...
function selectBest(snapshot, session) {
  ...
  evaluated.sort((a, b) => {
    ...
    const pa = a.action.type === 'trade' ? 3 : a.action.type === 'attack' ? 2 : a.action.type === 'gather' ? 1 : 0;
    const pb = b.action.type === 'trade' ? 3 : b.action.type === 'attack' ? 2 : b.action.type === 'gather' ? 1 : 0;
...
app.get('/profile', async () => ({
  ok: true,
  profile: PROFILE,
  notes: 'Control/economy endpoint with adaptive counter-strategy and stateful prediction'
}));
